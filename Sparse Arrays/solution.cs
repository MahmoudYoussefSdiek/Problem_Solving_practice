...
class Result
{
    ...
    public static List<int> matchingStrings(List<string> strings, List<string> queries)
    {
        ...
        // Create a list to store the count of each query string
        List<int> queryCounts = new List<int>();

        // Count the number of times each query string appears in the list
        foreach (string query in queries)
        {
            if (stringCounts.ContainsKey(query))
                queryCounts.Add(stringCounts[query]);
            else
                queryCounts.Add(0);
        }
